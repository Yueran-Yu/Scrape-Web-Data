...
noir_links = []
for i in range(1, 3):
    drama = requests.get('https://www.twilighttimemovies.com/genre/film-noir/?sort=featured&page=' + str(i))
    ...
    for item in items:
        ...
        if a is None:
            continue
        noir_links.append(a['href'])
        # print(a['href'])

id = 1

for line in noir_links:
    # print(line)
    ...
    movies_item_dict.update(
        {id: {'name': title, 'description': description, 'price': price, 'img_link': img_link, 'category': 'Film Noir'}})
    id += 1

pprint.pprint(movies_item_dict)

with open(os.path.join('Outputs', 'film_noir.json'), 'w') as fp:
