...
crime_links = []
for i in range(1, 3):
    drama = requests.get('https://www.twilighttimemovies.com/genre/crime/?sort=featured&page=' + str(i))
    ...
    for item in items:
        ...
        if a is None:
            continue
        crime_links.append(a['href'])
        # print(a['href'])

id = 1

for line in crime_links:
    # print(line)
    ...
    movies_item_dict.update(
        {id: {'name': title, 'description': description, 'price': price, 'img_link': img_link,  'category': 'Crime'}})
    id += 1

pprint.pprint(movies_item_dict)

with open(os.path.join('Outputs', 'crime.json'), 'w') as fp:
