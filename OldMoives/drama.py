...
doc_links = []
for i in range(1, 8):
    drama = requests.get('https://www.twilighttimemovies.com/genre/drama/?sort=featured&page=' + str(i))
...
for line in doc_links:
    ...
    movies_item_dict.update(
        {id: {'name': title, 'description': description, 'price': price, 'img_link': img_link,  'category': 'Drama'}})
    id += 1

pprint.pprint(movies_item_dict)

with open(os.path.join('Outputs', 'drama.json'), 'w') as fp:
